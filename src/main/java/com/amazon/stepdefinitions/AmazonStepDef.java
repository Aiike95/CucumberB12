...
@Then("User validates the number is {int} results on the top shown")
public void user_validates_the_number_is_results_on_the_top_shown(int expectedResult) {
    mainPage=new MainPage(driver);
    Assert.assertEquals(mainPage.resultOfSearch(),expectedResult);
        }
    @Then("User validates all headers contains iphone")
    public void user_validates_all_headers_contains_iphone() {
        mainPage=new MainPage(driver);
